...
listZoom = [1.2, 1.2, 1.6, 1.3, 1.9]  # zoom moyen qu'atteint chaque config (1 correspond à aucun zoom)
listPoints = [(960, 540), (1670, 250), (1733, 893), (230, 230),(1763, 157)]  # liste de coordonnées (colonne, ligne) des différents points d'interets
...
class ImageSequence():
    ...
    def extractFrames(self, videoPath):
        ...
        if os.path.isfile(videoPath):
            ...
            if self.frameCount:
                ...
                raise SystemError(f"error converting {videoPath} to frames")

        else:
            raise SystemError(f"file {videoPath} not found")
    ...
    def step(self, loop=False):
        ...
        if loop:  # loops back to the end and forth to the beginning
            ...
            if self.playingForward:
                if self.currentFrameIndex + 1 < self.frameCount - 1:
...
if __name__ == "__main__":
    ...
    while playing:
        ...
        if loopTime > changeConfigTimer and reachZoom and reachPoints:
            ...
            changeConfigTimer = getConfigTimer() + timedelta(seconds=newZoom * max(timeTransiZoom, timeTransiConfig))
        ...
        if headlessMode:
            if not reachZoom:
                ...
                if timeTransiZoom != 0:
                    deltaZoom = (newZoom - img.getZoom()) / (maxFPS * timeTransiZoom)
                else:
                    deltaZoom = 0
